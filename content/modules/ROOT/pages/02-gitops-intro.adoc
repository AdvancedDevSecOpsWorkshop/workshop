...
We want to deploy that application to two namespaces: a "development" environment and a production environement, that we will later on update using the DevSecOps pipeline.
...
Let's first deploy the application to the {user}-dev environment:

[.console-input]
[source,bash,subs="attributes+,+macros"]
----
sed 's/$USER/{user}/' ~/workshop/content/modules/ROOT/files/gitops/coolstore-gitops-dev.yaml | oc apply -n {user}-argocd -f -
----
...
[source,bash,subs="attributes+,+macros"]
----
sed 's/$USER/{user}/' ~/workshop/content/modules/ROOT/files/gitops/coolstore-gitops-prod.yaml | oc apply -n {user}-argocd -f -
----
...
image::gitops-apps/gitops-userx-dev.png[coolstore App]
