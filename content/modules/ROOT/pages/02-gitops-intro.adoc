...
Now let's login to OpenShift GitOps using this URL: link:https://argocd-server-gitops.{openshift_cluster_ingress_domain}[https://argocd-server-gitops.{openshift_cluster_ingress_domain},window="_blank"]
...
replicaset.apps/gateway-vertx-764dd8945          0         0         0       25m
...
Note that when selfHeal is not enabled you can manually sync your app via the Argo CD UI by:
