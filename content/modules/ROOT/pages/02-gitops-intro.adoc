...
comes into play. ApplicationSets enable you to generate Application resources using templating. Each ApplicationSet can include one or
...
spec:
  ...
  template: <4>
    ...
    spec:
      destination:
        name: in-cluster
        namespace: user1-{{.environment}}
      project: user1
      source:
        path: content/modules/ROOT/files/module-01
        repoURL: https://gitea-gitea.apps.cluster-7mrzs.7mrzs.sandbox310.opentlc.com/user1/workshop.git
        targetRevision: HEAD
      syncPolicy:
        automated:
          prune: true
...
In module 4 we will revisit ApplicationSets again but using the git generator.
