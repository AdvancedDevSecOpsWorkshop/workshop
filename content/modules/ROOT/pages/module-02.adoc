...
We wil use the ArgoCD CLI to explore the Shared GitOps in more detail. To do this login into Argo CD as follows
using the `--sso` switch to login using Keycloak.

[.console-input]
[source,sh,subs="attributes",role=execute]
----
argocd login --sso --grpc-web argocd-server-gitops.{openshift_cluster_ingress_domain}
----

[Note]
The `--sso` switch starts a local web server on localhost:8085 that is used to handle the callback from
...
'user{user}@workshop.com' logged in successfully
Context 'argocd-server-gitops.{openshift_cluster_ingress_domain}' updated
----
...
[source,bash,subs="attributes+,+macros"]
----
Name:               user{user}-argocd/dev-tools
Project:            user{user}
Server:             in-cluster
...
GROUP               KIND                   NAMESPACE    NAME                       STATUS  HEALTH   HOOK  MESSAGE
                    Secret                 user{user}-cicd   sonarqube-admin            Synced                 secret/sonarqube-admin created
                    PersistentVolumeClaim  user{user}-cicd   nexus                      Synced  Healthy        persistentvolumeclaim/nexus created
                    PersistentVolumeClaim  user{user}-cicd   sonarqube-data             Synced  Healthy        persistentvolumeclaim/sonarqube-data created
                    PersistentVolumeClaim  user{user}-cicd   postgresql-sonarqube-data  Synced  Healthy        persistentvolumeclaim/postgresql-sonarqube-data created
                    Service                user{user}-cicd   sonarqube                  Synced  Healthy        service/sonarqube created
                    Service                user{user}-cicd   nexus                      Synced  Healthy        service/nexus created
                    Service                user{user}-cicd   postgresql-sonarqube       Synced  Healthy        service/postgresql-sonarqube created
apps                Deployment             user{user}-cicd   nexus                      Synced  Healthy        deployment.apps/nexus created
apps                Deployment             user{user}-cicd   sonarqube                  Synced  Healthy        deployment.apps/sonarqube created
batch               Job                    user{user}-cicd   configure-nexus            Synced  Healthy        job.batch/configure-nexus created
batch               Job                    user{user}-cicd   configure-sonarqube        Synced  Healthy        job.batch/configure-sonarqube created
route.openshift.io  Route                  user{user}-cicd   nexus                      Synced  Healthy        route.route.openshift.io/nexus created
apps.openshift.io   DeploymentConfig       user{user}-cicd   postgresql-sonarqube       Synced  Healthy        deploymentconfig.apps.openshift.io/postgresql-sonarqube created
route.openshift.io  Route                  user{user}-cicd   sonarqube                  Synced  Healthy        route.route.openshift.io/sonarqube created
----
...
team{user}  team{user} project  5 destinations  *        <none>                      <none>                        <none>          disabled
----
...
----
Name:                        team{user}
Description:                 team{user} project
Destinations:                ,user{user}-argocd
                             ,user{user}-dev
                             ,user{user}-stage
                             ,user{user}-prod
                             ,user{user}-cicd
