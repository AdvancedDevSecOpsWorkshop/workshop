...
[source,sh,subs="attributes",role=execute]
----
argocd proj role get user1 admin
----
...
different link:https://argo-cd.readthedocs.io/en/stable/operator-manual/rbac/#rbac-resources-and-actions[actions, window="_blank"] are available.
5. Next is the specific resource, this can be a wildcard or a named resource such as `user` in this case
