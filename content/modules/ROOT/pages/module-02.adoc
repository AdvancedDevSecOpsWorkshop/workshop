...
3. Then the resource type for which we are giving permissions to, in this case `projects`. Many
...
different link:https://argo-cd.readthedocs.io/en/stable/operator-manual/rbac/#rbac-resources-and-actions[actions, window="_blank"] are available. A
wildcard of `*` indicates all actions.
...
So let's test this policy, as discussed we only have the ability to view this basic project but
we cannot modify or delete. Let's try it anyway:
