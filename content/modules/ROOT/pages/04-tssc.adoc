...
* Login using Keycloak SSO:

image::/devsecops/gitea-login-with-sso.png

* Select the `Link Account` tab, do not `Register New Account`. Link the existing account to Keycloak using the provided credentials: username: *{user}*, password: *openshift*.

image:;/devsecops/gitea-link-account.png
...
----
cat workshop/content/modules/ROOT/examples/devsecops-pipelinerun.yaml | sed 's/$USERNUM/workshop1/' | sed 's/$USER/user1/' | sed 's/$SUBDOMAIN/apps.cluster-gm7tc.gm7tc.sandbox2168.opentlc.com/' | oc create -f - -n {user}-cicd
----
