...
----
sed 's/$USER/{user}/;s/$WORKSHOP_NUM/workshop{usernum}/' ~/workshop/content/modules/ROOT/examples/devsecops-pipelinerun.yaml | oc apply -f - -n {user}-cicd
----

Let's review the content of the pipelineRun:
...
spec:
  params:
    ...
    - name: MANIFEST_REPO
      value: 'https://4fdb9ba7a4dbf754b9f06b744f06510874282bd1@gitea-gitea.apps.cluster-424xp.424xp.sandbox1774.opentlc.com/$USER/gitops'
...
* Let's see the same resources deployed in the application namespace *{user}-dev* with this URL: https://{openshift_console_url}/topology/ns/{user}-dev?view=graph
