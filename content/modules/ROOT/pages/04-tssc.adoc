...
This verification is performed every time the image is built, however in the next section we will look at how we can inform and enforce a signature requirement through a Policy.
...
image::/devsecops/trusted-signature-policy.png[]

The Policy Details displays all of the policy related information:

image::/devsecops/acs-policy-details-readonly.png[]

In the "Policy details" section the metadata and the Severity level and some other information is defined.

image::acs-policy-details-section.png[]
