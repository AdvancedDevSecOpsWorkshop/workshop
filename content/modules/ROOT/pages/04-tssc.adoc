...
* Navigate to the file *content/module/ROOT/examples/devsecops-pipelinerun.yaml* and click on edit (the pen on the right side) to make changes. In the *params* section, look for *MANIFEST_REPO* and replace the *REPLACE_WITH_YOUR_TOKEN_VALUE* with the value you copy-pasted before.
...
[source,bash,subs="attributes+,+macros"]
----
cd && wget https://gitea-gitea.{openshift_cluster_ingress_domain}/{user}/workshop/raw/branch/main/content/modules/ROOT/examples/devsecops-pipeline.yaml && 
sed 's/$USER/{user}/;s/$WORKSHOP_NUM/WORKSHOP_XX/' ~/devsecops-pipelinerun.yaml | oc apply -f - -n {user}-cicd
----

Let's review the content of the pipelineRun. These are the main parameters that are used by the pipeline tasks to deploy the application:
