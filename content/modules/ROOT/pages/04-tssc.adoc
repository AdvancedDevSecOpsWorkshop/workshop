...
[source,sh,subs="attributes",role=execute]
----
sed 's/$USER/{user}/' ~/workshop/content/modules/ROOT/examples/web-nodejs-dev.yaml | oc apply -f - -n {user}-argocd
----
...
[source,sh,subs="attributes",role=execute]
----
sed 's/$USER/{user}/' ~/workshop/content/modules/ROOT/examples/web-nodejs-prod.yaml | oc apply -f - -n {user}-argocd
----
