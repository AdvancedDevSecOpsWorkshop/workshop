...
[NOTE]
The switch `--insecure-ignore-tlog=true` is added so that cosign bypasses the checking the public transparency log. Tekton Chains is configured to use the transparency log deployed on the workshop cluster, as a result the signature is never uploaded to the public transparency log that cosign uses by default.
