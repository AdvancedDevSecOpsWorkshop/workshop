...
+
6. Instead of using the default `coolstore` application, create a new one:
+
image::ocp-app/create-application.png[width=80%]
+
7. Name the new application `catalog-app`:
+
image::ocp-app/create-catalog-app.png[width=80%]
+
8. On Deploy -> Resource Type, select *Serverless Deployment*
9. Click on *Show advanced Deployment option*: and add *an environment variable* to connect with the Gateway URL:
